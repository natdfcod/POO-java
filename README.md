...
Um projeto prático de Programação Orientada a Objetos (POO) desenvolvido em Java, focado em simular as operações básicas de uma carteira com suporte a **múltiplas criptomoedas** (como Bitcoin e Ethereum). O sistema aplica regras de negócio reais, como cobrança de taxas de operação, diferentes categorias de clientes, processamento de pagamentos em Reais e consumo de dados do mercado financeiro.
...
O sistema simula transações de ativos digitais (`saldoCrypto`) com as seguintes capacidades:

* **Depósito:** Adição de fundos à carteira, com validação de valores positivos.
* **Saque:** Retirada de fundos com aplicação de uma taxa fixa de `0.01` da moeda correspondente por operação.
* **Transferência:** Envio de saldo entre diferentes contas, utilizando a lógica de saque da conta origem e depósito na conta destino. Inclui uma **trava de segurança** que impede a transferência entre moedas distintas (ex: Bitcoin para Ethereum).
* **Contas VIP:** Implementação de uma `CarteiraVip` que isenta o titular de taxas de saque.
* **Processamento de Pagamentos:** Simulação de depósito na corretora pagando em Reais (BRL), suportando diferentes regras de negócio (PIX sem taxas e Cartão de Crédito com taxa de 2%).
* **Cotação Híbrida (API e Manual):** Conversão automática do dinheiro depositado em Reais para frações da criptomoeda. Para Bitcoin, consome o preço em tempo real via API pública da corretora Binance. Para outras moedas, permite a inserção manual da cotação do momento via console.
* **Extrato:** Exibição formatada no console do titular, da moeda operada e do saldo atual.
...
* **Interfaces:** Criação de contratos (`MetodoPagamento`) para garantir que diferentes formas de pagamento (`PagamentoPix`, `PagamentoCartao`) implementem obrigatoriamente as mesmas assinaturas de processamento.
